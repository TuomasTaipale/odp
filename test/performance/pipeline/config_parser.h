...
uintptr_t config_parser_get_resource(const char *domain, const char *resource);

void config_parser_register_parser(const char *domain, conf_init_fn_t init_fn,
				   conf_deploy_fn_t deploy_fn, conf_destroy_fn_t destroy_fn,
				   conf_resource_fn_t resource_fn);

void config_parser_destroy(void);

#define CONFIG_PARSER_AUTOREGISTER(prio, domain, init, deploy, destroy, resource)	\
	__attribute__((constructor(prio)))						\
	static void CONCAT(autoregister, __LINE__)(void) {				\
		config_parser_register_parser(domain, init, deploy, destroy, resource);	\
	}
