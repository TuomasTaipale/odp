...
#define CIPHER_SNOW_V_GCM "snow_v_gcm"
...
#define AUTH_SNOW_V_GCM "snow_v_gcm"
...
static odp_bool_t parse_crypto_entry(config_setting_t *cs, crypto_parse_t *crypto)
{
	...
	if (config_setting_lookup_string(cs, CONF_STR_CIPHER_ALG, &val_str) == CONFIG_TRUE) {
		...
		} else if (strcmp(val_str, CIPHER_SNOW_V_GCM) == 0) {
	...
	if (config_setting_lookup_string(cs, CONF_STR_AUTH_ALG, &val_str) == CONFIG_TRUE) {
		...
		} else if (strcmp(val_str, AUTH_SNOW_V_GCM) == 0) {
...
CONFIG_PARSER_AUTOREGISTER(LOW_PRIO, CRYPTO_DOMAIN, crypto_parser_init, crypto_parser_deploy,
