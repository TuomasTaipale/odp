...
static odp_bool_t parse_timer_entry(config_setting_t *cs, timer_parse_t *timer)
{
	...
	if (config_setting_lookup_int(cs, CONF_STR_CLK_SRC, &val_i) == CONFIG_TRUE)
		timer->param.clk_src = val_i;
	...
	if (config_setting_lookup_int64(cs, CONF_STR_MIN_TMO, &val_ll) == CONFIG_TRUE)
...
CONFIG_PARSER_AUTOREGISTER(MED_PRIO, TIMER_DOMAIN, timer_parser_init, timer_parser_deploy,
