...
The passed configuration is divided into domains which the application parses at the beginning.
The domains follow ODP modules (in addition to application specific domains), e.g. there is a
...
Application will abort if a name lookup error is encountered as this typically represents a fatal
...
- `pktios` entries:
  ...
  - element: `inmode`
    - necessity: optional
    - type: string
    - values: `"queue"`, `"schedule"`, mapping to `odp_pktin_mode_t`
    - default: `"queue"`
  - element: `outmode`
    - necessity: optional
    - type: string
    - values: `"queue"`, mapping to `odp_pktout_mode_t`
    - default: `"queue"`
...
which in turn is part of a flow and flows are then finally attached to a queue. Framework then
...
consumed and framework moves to poll/schedule additional events. If events remain unconsumed after
a flow has been fully executed, they are simply freed.

Framework also supports event generation or generic code execution through output flows, where a
