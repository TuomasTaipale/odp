...
- name in configuration file: `dma_segmentation`
- type: input
- info: distributes a configurable amount of fragments of larger packets across a given set of
        queues, distribution is based on a fragment ID found in a configurable offset of received
	packets, after the configured amount of fragments are received in a flow, the fragments are
	reassembled to a larger packet with DMA
- parameter list:
  - index 0: offset to a packet to find fragment ID
    - type: integer (single byte value space expected)
  - index 1: fragment queue resource names
    - type: array
  - index 2: dma session resource names
    - type: array
  - index 3: DMA completion queue resource name
    - type: string
  - index 4: DMA destination packet pool resource name
    - type: string
  - index 5: DMA completion event pool resource name
    - type: string
  - index 6: DMA transfer inflight data stash resource name (8 byte objects expected)
    - type: string
  - index 7: Fragment count
    - type: integer (single byte value space expected)

- name in configuration file: `dma_completion`
- type: input
- info: forwards DMA reassembled packets to a set of queues
- parameter list:
  - index 0: LSO toggle
    - type: integer (boolean value space expected)
  - index 1: if LSO disabled, queue resource names
    - type: array
  - index 1: if LSO enabled, packet I/O resource name
    - type: string
  - index 2: LSO payload offset
    - type: integer
  - index 3: LSO payload size
    - type: integer
  - index 4: LSO number of direct packet I/O queues
    - type: integer
