...
static odp_bool_t parse_pool_entry(config_setting_t *cs, pool_parse_t *pool)
{
	...
	odp_pool_param_init(&pool->param);
	...
	if (strcmp(val_str, PACKET) == 0) {
		pool->param.type = ODP_POOL_PACKET;
	} else if (strcmp(val_str, BUFFER) == 0) {
		pool->param.type = ODP_POOL_BUFFER;
	} else if (strcmp(val_str, TIMEOUT) == 0) {
		pool->param.type = ODP_POOL_TIMEOUT;
	} else {
	...
	if (config_setting_lookup_int(cs, CONF_STR_CACHE_SIZE, &val_i) == CONFIG_TRUE) {
		pool->param.buf.cache_size = val_i;
		pool->param.pkt.cache_size = val_i;
		pool->param.tmo.cache_size = val_i;
	}
	...
	if (pool->param.type != ODP_POOL_TIMEOUT) {
	...
	if (pool->param.type == ODP_POOL_PACKET) {
		pool->param.pkt.len = size;
		pool->param.pkt.seg_len = size;
		pool->param.pkt.num = num;
	} else if (pool->param.type == ODP_POOL_BUFFER) {
		pool->param.buf.size = size;
		pool->param.buf.num = num;
	} else {
		pool->param.tmo.num = num;
	}
...
static odp_bool_t pool_parser_deploy(void)
{
	...
	for (uint32_t i = 0U; i < pools.num; ++i) {
		pool = &pools.pools[i];
		pool->pool = odp_pool_create(pool->name, &pool->param);
...
CONFIG_PARSER_AUTOREGISTER(HIGH_PRIO, POOL_DOMAIN, pool_parser_init, pool_parser_deploy,
