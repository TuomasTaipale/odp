 ...
 * with the requested values: For ODP_TIMER_TYPE_PERIODIC timers, timer pool base frequency and
 * the maximum frequency multiplier. For ODP_TIMER_TYPE_PERIODIC_2 timers, a pointer to an array of
 * frequencies and the number of items in the array. The minimum timeout resolution can
 * additionally be set for both. If there is no requirement for timeout resolution, it is set to
 * zero. Type is chosen with odp_timer_periodic_capability_t::type.
 *
 ...
 * (odp_timer_periodic_capability_t::type_1::base_freq_hz or each frequency in
 * odp_timer_periodic_capability_t::type_2::freq_hz array), and meets or exceeds other values.
 * Frequency values are not modified, but other potential 'capa' fields are updated with resulting
 ...
 *
 * @retval 1   Success. Capability matches frequency, and meets or exceeds other requested values.
 * @retval 0   Success. Capability does not match frequency exactly, but meets or exceeds other
 *             requested values.
 * @retval <0  Failure
 */
 ...
 * been created with ODP_TIMER_TYPE_PERIODIC/ODP_TIMER_TYPE_PERIODIC_2 type. The allocated timer is
 * started with odp_timer_periodic_start() call. A timer may be reused multiple times before
 * freeing it back into the timer pool.
 *
 * @param timer_pool Timer pool
 ...
 * delivers any remaining event(s) (may not arrive at the requested interval) to the destination
 * queue and return value of 1 marks the last of these delivered events. After application has
 * acknowledged it and all other events from the timer, it can free or start the timer again.
 *
 ...
 * may not arrive at the requested interval. Return value of odp_timer_periodic_ack() call will
 * indicate the last timeout event delivered by the timer.
 *
 * @param      timer  Timer
 *
