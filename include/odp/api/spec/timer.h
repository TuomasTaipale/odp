 ...
 *
 * When timer expires, a timeout event is sent into the provided destination queue. For single shot
 * timers, the event is configured with odp_timer_start_t::tmo_ev. With periodic timers, the event
 * is from an implementation created timeout pool, configured with
 * odp_timer_pool_param_t::periodic::tmo_pool. Additionally, with periodic timers and
 * non-atomic-scheduled destination queues, source ordering of timeouts may change due to
 * application processing order. In this case, it is up to the application to ensure acknowledgment
 * of all timeouts.
 *
 ...
 * filling it up if application falls behind.
 *
 ...
 * timer can be freed or started again.
 *
 * Application can distinguish timeouts associated with a periodic timer with
 * odp_timer_is_periodic().
 *
