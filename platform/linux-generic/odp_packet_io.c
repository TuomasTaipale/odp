...
int _odp_pktio_init_global(void)
{
	...
	pktio_global->ws_index = _odp_qpj_reserve_ws();

	if (pktio_global->ws_index < 0) {
		_ODP_ERR("Failed to reserve work slot for queue poll jobs: %d\n",
			 pktio_global->ws_index);
		return -1;
	}
...
static int pktin_deq_wd_multi(odp_queue_t queue, _odp_event_hdr_t *event_hdr[], int num,
			      void *data, int *num_out)
{
	int nbr = 0;
	_odp_event_hdr_t *hdr_tbl[QUEUE_MULTI_MAX];
	int pkts, i, j;
	odp_pktin_queue_t *pktin_queue = data;
...
int odp_pktio_start(odp_pktio_t hdl)
{
	...
	if (mode == ODP_PKTIN_MODE_SCHED || mode == ODP_PKTIN_MODE_QUEUE) {
		for (uint32_t i = 0; i < entry->num_in_queue; i++) {
			_odp_qpj_init_ws(&entry->in_queue[i].ws_data, pktin_deq_wd_multi,
					 &entry->in_queue[i].pktin, pktio_global->ws_index, 0);
			if (_odp_queue_fn->add_poll_job(entry->in_queue[i].queue,
							&entry->in_queue[i].ws_data) < 0) {
				_ODP_ERR("Failed to add poll job\n");
				return -1;
			}
		}
	}
