...
#pragma GCC diagnostic pop
	...
	struct {
		...
		odp_cpumask_t mask;
		odp_queue_t job_q[ODP_THREAD_COUNT_MAX];
	} centralized;
...
static void *schedule_service(void *args);
...
static void create_service_threads(void)
{
	...
	odp_atomic_init_u32(&sched->centralized.num, 0);
	...
	for (uint32_t i = 0; i < sched->centralized.num_threads; i++) {
		...
		ret = pthread_create(&sched->centralized.threads[i].thread,
				     ...
				     schedule_service, &sched->centralized.is_running);
...
static void sched_local_init(void)
{
	...
	if (!sched->centralized.is_init) {
		sched->centralized.is_init = true;
	...
	sched->centralized.job_q[thr] = proxy->job_q;
	odp_atomic_inc_u32(&sched->centralized.num);
...
static int schedule_term_global(void)
{
	...
	if (odp_global_rw->schedule_configured == 1) {
		...
		for (uint32_t k = 0; k < sched->centralized.num_threads; k++) {
			(void)pthread_join(sched->centralized.threads[k].thread, NULL);
			(void)pthread_attr_destroy(&sched->centralized.threads[k].thread_attr);
		}

		for (uint32_t k = 0; k < odp_atomic_load_u32(&sched->centralized.num); k++)
			(void)odp_queue_destroy(sched->centralized.job_q[k]);
...
static void *schedule_service(void *args)
{
	odp_atomic_u32_t *is_running = (odp_atomic_u32_t *)args, *num = &sched->centralized.num;
	odp_queue_t job_q;
	uint32_t idx = 0;
	uint64_t now;
	odp_event_t ev;
	...
	if (odp_init_local((odp_instance_t)odp_global_ro.main_pid, ODP_THREAD_WORKER) < 0)
	...
	while (odp_atomic_load_u32(is_running) == 1) {
		timer_run(1);
		job_q = sched->centralized.job_q[idx++];

		if (idx >= odp_atomic_load_u32(num))
			idx = 0;
		...
		if (ret == 0 && ((work->timeout_ns != ODP_SCHED_NO_WAIT && now < work->timeout_ns) || work->timeout_ns == ODP_SCHED_WAIT)) {
			if (odp_queue_enq(job_q, ev) == 0)
				continue;
			else
				...
				odp_event_t out_ev[], uint32_t max_num)
{
	...
	while (1) {
		ret = do_schedule(out_queue, out_ev, max_num, NULL);
		if (ret) {
			timer_run(2);
			break;
		}
