				       ...
				       odp_ipsec_op_status_t *status, uint32_t *orig_ip_len)
{
	...
	/* FIXME: maybe use packet flag as below ??? This adds requirement that input packets
	 * contain not only valid l3/l4 offsets, but also valid packet flags
...
static int ipsec_in_check_crypto_res(const odp_crypto_packet_result_t *crypto,
				     odp_ipsec_op_status_t *status)
{
	if (odp_likely(crypto->ok))
		return 0;
...
static int ipsec_do_crypto(odp_packet_t *pkt, odp_crypto_packet_op_param_t *param,
			   odp_ipsec_op_status_t *status,
			   int (*err_checker)(const odp_crypto_packet_result_t *crypto,
					      odp_ipsec_op_status_t *status))
{
	...
	return err_checker(&crypto, status);
				   ...
				   uint32_t *orig_ip_len)
{
	...
	if (odp_unlikely(ipsec_in_parse_encap_packet(pkt, &state, status, orig_ip_len) ||
	...
	if (ipsec_do_crypto(&pkt, &param, status, ipsec_in_check_crypto_res))
		goto exit;
	...
	if (odp_unlikely(_odp_ipsec_sa_lifetime_update(ipsec_sa, state.stats_length,
						       status) < 0) ||
...
static int ipsec_out_parse_tp(odp_packet_t pkt, ipsec_state_t *state)
{
	int (*op)(ipsec_state_t *state, odp_packet_t pkt);
...
static int ipsec_out_parse_tunnel(odp_packet_t *pkt, ipsec_state_t *state, ipsec_sa_t *ipsec_sa,
				  const odp_ipsec_out_opt_t *opt)
{
	int ret;
					...
					odp_ipsec_op_status_t *status)
{
	...
	if (ODP_IPSEC_MODE_TRANSPORT == ipsec_sa->mode)
		ret = ipsec_out_parse_tp(*pkt, state);
	else
		ret = ipsec_out_parse_tunnel(pkt, state, ipsec_sa, opt);
				...
				odp_crypto_packet_op_param_t *param, odp_ipsec_op_status_t *status)
{
	...
	if (ODP_IPSEC_ESP == ipsec_sa->proto)
		ret = ipsec_out_esp(pkt, state, ipsec_sa, param, status, mtu, is_enqueue_op,
				    opt);
	else
...
static int ipsec_out_check_crypto_res(const odp_crypto_packet_result_t *crypto,
				      odp_ipsec_op_status_t *status)
{
	if (odp_likely(crypto->ok))
		return 0;
...
static int ipsec_out_finalize_op(odp_packet_t *pkt, ipsec_state_t *state, ipsec_sa_t *ipsec_sa,
				 odp_ipsec_op_status_t *status)
{
				    ...
				    odp_ipsec_op_status_t *status)
{
	...
	if (odp_unlikely(ipsec_out_parse_encap_packet(&pkt, &state, ipsec_sa, opt, status) ||
			 ipsec_out_prepare_op(&pkt, &state, ipsec_sa, opt, enqueue_op, &param,
					      status)))
		goto exit;
	...
	if (odp_unlikely(ipsec_do_crypto(&pkt, &param, status, ipsec_out_check_crypto_res)))
		goto post_lifetime_err_cnt_update;

	if (odp_unlikely(ipsec_out_finalize_op(&pkt, &state, ipsec_sa, status)))
		goto post_lifetime_err_cnt_update;
