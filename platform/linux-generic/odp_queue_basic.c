...
static int queue_destroy(odp_queue_t handle)
{
	...
	_odp_qpj_destroy_jobs(&queue->jobs);
	UNLOCK(queue);
				 ...
				 _odp_event_hdr_t *event_hdr[], int num)
{
	...
	if (num_deq < num) {
		num_deq = _odp_qpj_poll(&qentry_from_handle(handle)->jobs, handle, &event_hdr[ret],
					num - ret);
		ret += num_deq < 0 ? 0 : num_deq;
	}
...
int _odp_sched_queue_deq(uint32_t queue_index, odp_event_t ev[], int max_num)
{
	...
	if (!_odp_qpj_has_jobs(&queue->jobs)) {
	...
	num_deq = _odp_qpj_poll(&queue->jobs, queue->handle, (_odp_event_hdr_t **)ev, max_num);

	if (num_deq <= 0) {
		LOCK(queue);

		if (num_deq == _QPJ_DONE && queue->status == QUEUE_STATUS_SCHED)
			queue->status = QUEUE_STATUS_NOTSCHED;
...
int _odp_sched_queue_empty(uint32_t queue_index)
{
	...
	if (ring_st_is_empty(&queue->ring_st) && !_odp_qpj_has_jobs(&queue->jobs)) {
...
static int queue_add_poll_job(odp_queue_t handle, _odp_qpj_poll_job_t *job)
{
	queue_entry_t *queue = qentry_from_handle(handle);

	_odp_qpj_add(&queue->jobs, job);

	if (queue->type == ODP_QUEUE_TYPE_SCHED) {
		_odp_sched_queue_set_status(queue->index, QUEUE_STATUS_SCHED);

		if (_odp_sched_fn->sched_queue(handle)) {
			_odp_qpj_rem(&queue->jobs, job);
			return -1;
		}
	}
