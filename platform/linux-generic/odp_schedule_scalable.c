		 ...
		 uint32_t actual, bool atomic)
{
	qschedstate_t oss, nss;
	uint32_t ticket;
	uint32_t num_jobs = odp_atomic_load_u32(&((queue_entry_t *)q)->num_jobs);
	...
	if (atomic) {
		...
		do {
			...
			nss = oss;
			nss.numevts -= actual;
			if ((nss.numevts > 0  || num_jobs > 0) && !pushed) {
				schedq_push(q->schedq, q);
				pushed = true;
			}
	...
	if (odp_unlikely(ticket != TICKET_INVALID)) {
		 ...
		 */
		if (oss.numevts > 0 && nss.numevts <= 0 && num_jobs == 0) {
		    ...
		    uint32_t actual, bool atomic)
{
	...
	if (atomic) {
		...
		if (q->qschst.numevts > 0 || num_jobs > 0)
			schedq_push(q->schedq, q);
		return;
	}
